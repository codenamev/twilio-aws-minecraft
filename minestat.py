...
# http://www.dilley.me/
#
# Modified slightly to correctly parse motd and add logger:
# Valentino Stoll
#
...
class MineStat:
  ...
  def __init__(self, address, port, timeout = DEFAULT_TIMEOUT, logger = logging.getLogger()):
    ...
    try:
      ...
      logger.debug("MineStat Error: " + str(error))
      self.online = False
    ...
    if raw_data is None or raw_data == '':
      ...
      if data and len(data) >= self.NUM_FIELDS:
        ...
        self.motd = (data[3] or '').replace("\x00", "")
